...
class RemoteCmdsStrategy(StrategyPyBase):
    """
    ...
    def __init__(self, exchange: ExchangeBase, trading_pair: str, asset: str):

        super().__init__()
        self._exchange = exchange
        self._asset = asset
        self._ready = False
        # Remote Commands: Setup delegate instance
        self._remote_commands_delegate = RemoteCommandsDelegate(self)
    ...
    async def format_status(self) -> str:
        ...
        lines.extend(
            ["", "  Assets:"] + ["    " + str(self._asset) + "    " + str(self._exchange.get_balance(self._asset))]
        )
    ...
    def start(self, clock: Clock, timestamp: float):
        # Remote Commands: Register Events on start
        self._remote_commands_delegate.register_events()

    def stop(self, clock: Clock):
        # Remote Commands: Unregister Events on stop
        self._remote_commands_delegate.unregister_events()
