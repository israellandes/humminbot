...
main_config_map = {
    ...
    "instance_id": ConfigVar(key="instance_id", prompt=None, required_if=lambda: False, default=generate_client_id()),
    ...
    "logger_override_whitelist": ConfigVar(
        ...
        default=["hummingbot.strategy", "conf"],
        type_str="list",
    ),
    "key_file_path": ConfigVar(
        key="key_file_path",
        prompt=f"Where would you like to save your private key file? " f"(default '{DEFAULT_KEY_FILE_PATH}') >>> ",
        required_if=lambda: False,
    ...
    "ethereum_rpc_ws_url": ConfigVar(
        key="ethereum_rpc_ws_url",
    ...
    "ethereum_chain_name": ConfigVar(
        key="ethereum_chain_name",
    ...
    "kill_switch_rate": ConfigVar(
        key="kill_switch_rate",
        prompt="At what profit/loss rate would you like the bot to stop? " "(e.g. -5 equals 5 percent loss) >>> ",
        type_str="decimal",
        default=-100,
    ...
    "autofill_import": ConfigVar(
        key="autofill_import",
        prompt="What to auto-fill in the prompt after each import command? (start/config) >>> ",
        type_str="str",
        default=None,
        validator=lambda s: None if s in {"start", "config"} else "Invalid auto-fill prompt.",
        required_if=lambda: False,
    ),
    "telegram_enabled": ConfigVar(
    ...
    "telegram_token": ConfigVar(
        key="telegram_token", prompt="What is your telegram token? >>> ", required_if=lambda: False
    ),
    "telegram_chat_id": ConfigVar(
        key="telegram_chat_id", prompt="What is your telegram chat id? >>> ", required_if=lambda: False
    ),
    "remote_commands_enabled": ConfigVar(
        key="remote_commands_enabled",
        prompt="Would you like to enable remote commands? >>> ",
        type_str="bool",
        required_if=lambda: False,
        default=False,
    ),
    "remote_commands_api_key": ConfigVar(
        key="remote_commands_api_key",
        prompt="What is your remote commands api key? >>> ",
        required_if=lambda: global_config_map["remote_commands_enabled"].value,
    ),
    "remote_commands_ws_url": ConfigVar(
        key="remote_commands_ws_url",
        prompt="What is your remote commands websocket url? >>> ",
        required_if=lambda: global_config_map["remote_commands_enabled"].value,
    ),
    "remote_commands_routing_name": ConfigVar(
        key="remote_commands_routing_name",
        prompt="What is your remote commands routing name? >>> ",
        required_if=lambda: False,
    ),
    "remote_commands_ignore_first_event": ConfigVar(
        key="remote_commands_ignore_first_event",
        prompt="Whether to ignore the first event received? >>> ",
        required_if=lambda: False,
        type_str="bool",
        default=True,
    ),
    "remote_commands_disable_console_commands": ConfigVar(
        key="remote_commands_disable_console_commands",
        prompt="Whether to disable console command processing? >>> ",
        required_if=lambda: False,
        type_str="bool",
        default=False,
    ),
    "remote_commands_translate_commands": ConfigVar(
        key="remote_commands_translate_commands",
        prompt="How to translate received commands to Hummingbot commands? >>> ",
        required_if=lambda: False,
        type_str="json",
    ),
    "send_error_logs": ConfigVar(
        key="send_error_logs",
    ...
    "balance_asset_limit": ConfigVar(
        key="balance_asset_limit",
        prompt="Use the `balance limit` command" "e.g. balance limit [EXCHANGE] [ASSET] [AMOUNT]",
        required_if=lambda: False,
        type_str="json",
        default={exchange: None for exchange in AllConnectorSettings.get_exchange_names()},
    ),
    "manual_gas_price": ConfigVar(
    ...
    "gateway_api_host": ConfigVar(key="gateway_api_host", prompt=None, required_if=lambda: False, default="localhost"),
    "gateway_api_port": ConfigVar(
        key="gateway_api_port",
        prompt="Please enter your Gateway API port >>> ",
        type_str="str",
        required_if=lambda: False,
        default="5000",
    ),
    "heartbeat_enabled": ConfigVar(
    ...
    "heartbeat_interval_min": ConfigVar(
    ...
    "command_shortcuts": ConfigVar(key="command_shortcuts", prompt=None, required_if=lambda: False, type_str="list"),
    "rate_oracle_source": ConfigVar(
        key="rate_oracle_source",
    ...
    "global_token_symbol": ConfigVar(
        key="global_token_symbol",
    ...
    "rate_limits_share_pct": ConfigVar(
        key="rate_limits_share_pct",
    ...
    "create_command_timeout": ConfigVar(
        key="create_command_timeout",
        prompt="Network timeout when fetching the minimum order amount" " in the create command (in seconds)  >>> ",
        type_str="decimal",
        validator=lambda v: validate_decimal(v, min_value=Decimal("0"), inclusive=False),
        required_if=lambda: False,
        default=Decimal("10"),
    ),
    "other_commands_timeout": ConfigVar(
        key="other_commands_timeout",
...
color_config_map = {
    ...
    "terminal-primary": ConfigVar(
        key="terminal-primary",
...
paper_trade_config_map = {
    "paper_trade_exchanges": ConfigVar(
        key="paper_trade_exchanges",
        prompt=None,
        required_if=lambda: False,
        default=[
            "binance",
    ...
    "paper_trade_account_balance": ConfigVar(
        key="paper_trade_account_balance",
