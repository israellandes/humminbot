...
color_settings_to_display = ["top-pane", "bottom-pane", "output-pane", "input-pane", "logs-pane", "terminal-primary"]
...
class ConfigCommand:
    ...
    def list_configs(
        ...
        if self.strategy_name is not None:
            data = [
                [cv.printable_key or cv.key, cv.value] for cv in self.strategy_config_map.values() if not cv.is_secure
            ]
            df = map_df_to_str(pd.DataFrame(data=data, columns=columns))
    ...
    async def _config_single_key(
        ...
        try:
            ...
            if isinstance(self.strategy, PureMarketMakingStrategy) or isinstance(
    ...
    async def inventory_price_prompt(
        ...
        if input_value:
            ...
            if exchange.endswith("paper_trade"):
                ...
                balances = await UserBalances.instance().balances(exchange, base_asset, quote_asset)
